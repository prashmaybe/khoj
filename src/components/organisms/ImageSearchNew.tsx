...
const ImageSearch: React.FC<ImageSearchProps> = ({ visible, onClose }) => {
  ...
  const [searchOptions, setSearchOptions] = useState<ImageSearchOptions>({
    query: '',
    provider: 'google',
    safeSearch: false,
    imageSize: 'all',
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const searchResults = await imageSearchService.getSearchResults(searchOptions);
      setResults(searchResults);
      imageSearchService.addToSearchHistory(query, searchOptions.provider);
    } catch (error) {
    ...
    >
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.searchContainer, { backgroundColor: colors.surface }]}>
          ...
          <TouchableOpacity
            style={[styles.optionsButton, { backgroundColor: colors.buttonSecondary }]}
            onPress={() => setShowOptions(!showOptions)}
          >
            <FiFilter size={20} color={colors.buttonSecondaryText} />
          </TouchableOpacity>
        </View>
                ...
                >
                  <Image
                    ...
                    style={[
                      styles.thumbnail,
                      {
                        width: viewMode === 'grid' ? windowWidth / 2 - 20 : 60,
